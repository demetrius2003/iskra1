...
│ emotional_valence │ от −1 до +1: насколько переживание при записи было приятным / неприятным   │
│ arousal           │ 0…1: насколько интенсивным было переживание (спокойное vs возбуждённое)   │
└──────────────┴─────────────────────────────────────────────────┘
```
...
При запуске пользователь может подключить «стартовые воспоминания» из YAML-файла (путь — `config.memory.initial_memories_file`). Они задают «личность» системы. Чтобы одни и те же строки не множились при каждом рестарте, после успешной загрузки в `general.data_dir` сохраняется маркер `.iskra_seed_marker.json` (абсолютный путь к YAML и SHA-256 его содержимого); при совпадении маркера загрузка пропускается. Изменили seed-файл — новый хеш; при необходимости удалите маркер или очистите хранилище памяти.
